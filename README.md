...
Colab에서 노트북을 여는 게 번거롭거나 GitHub 연동이 막히면, [`colab_paste.py`](colab_paste.py)를 쓰세요. 셀 3개로 나뉘어 있고 각 덩어리를 Colab 셀에 붙여 넣으면 됩니다.

- **파일 업로드 창이 뜨지 않습니다.** 참조 사진을 이 저장소의 raw URL에서 바로 받아옵니다.
- 셀 1은 설치, 셀 2는 준비(골격 추출·모델 로드·번역기), 셀 3이 작업 칸입니다.
- **셀 3만 고쳐서 반복 실행**하면 됩니다. 모델은 이미 올라가 있어 재실행이 빠릅니다.
- 셀 4는 결과를 빼내는 방법입니다. Colab은 런타임이 끊기면 파일이 사라지니, 만든 직후 zip으로 내려받거나 Google Drive에 저장하세요. GitHub로 바로 푸시하는 방법도 적어 뒀습니다.
